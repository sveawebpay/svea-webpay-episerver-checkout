...
using Svea.WebPay.Episerver.Checkout.Steps;
...
{
    public class SveaWebPayCheckoutGateway : ISplitPaymentPlugin, ISplitPaymentGateway, IPaymentPlugin
    {
        private static readonly ILogger Logger = LogManager.GetLogger(typeof(SveaWebPayCheckoutGateway));
        ...
        internal Injected<SveaWebPayClientFactory> InjectedSveaWebPayClientFactory { get; set; }
        private SveaWebPayClientFactory SveaWebPayClientFactory => InjectedSveaWebPayClientFactory.Service;
        ...
        public PaymentStepResult ProcessPayment(IPayment payment, IShipment shipment)
        {
            ...
            try
            {
                Logger.Debug("Svea WebPay checkout gateway. Processing Payment ....");

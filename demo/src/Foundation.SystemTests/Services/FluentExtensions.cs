...
namespace Foundation.SystemTests.Services
{
    public static class FluentExtensions
    {
        ...
        public static TOwner StorePrice<TOwner>(this UIComponent<TOwner> component, out decimal price, string characterToRemove = null)
            where TOwner : PageObject<TOwner>
        {
            var content = characterToRemove != null ? component.Content.Value.Replace(characterToRemove, "") : component.Content.Value;

            price = Convert.ToDecimal(RegexAmount.Match(content).Groups[1].Value.Replace(",", "."), new CultureInfo("en-US"));
            return component.Owner;
        }
